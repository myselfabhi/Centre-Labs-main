...
export default function CancelledOrdersPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<string>('CANCELLED');
  ...
  const handleExportAll = async () => {
    ...
    const fileName = `cancelled-orders-all-${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(wb, fileName);
    toast.success(`Exported ${all.length} cancelled orders to ${fileName}`);
  };
      ...
      <DashboardLayout>
        <div className="space-y-3 sm:space-y-6 px-2 sm:px-0">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Cancelled Orders</h1>
              <p className="text-muted-foreground text-xs sm:text-sm lg:text-base">
                Manage all cancelled orders
              </p>
            </div>
          ...
          <Card className="overflow-hidden">
            <CardHeader className="px-3 sm:px-6">
              ...
              <CardDescription>Search and filter cancelled orders</CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader className="px-3 sm:px-6">
              <CardTitle className="text-lg sm:text-xl">Cancelled Orders</CardTitle>
            </CardHeader>
            <CardContent className="px-3 sm:px-6">
