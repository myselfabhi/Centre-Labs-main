...
import { Plus, FileSpreadsheet, Search, XCircle } from 'lucide-react';
import { api, Order } from '@/lib/api';
import { toast } from 'sonner';
import * as XLSX from 'xlsx';

export default function FailedPaymentsOrdersPage() {
  ...
  useEffect(() => {
    fetchOrders();
  }, [currentPage, searchTerm, customerTypeFilter, dateRange]);
  ...
  const handleExportAll = async () => {
    ...
    const fileName = `failed-payments-orders-all-${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(wb, fileName);
    toast.success(`Exported ${all.length} failed-payment orders to ${fileName}`);
  };
      ...
      <DashboardLayout>
        <div className="space-y-3 sm:space-y-6 px-2 sm:px-0">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight">Failed Payments</h1>
              <p className="text-muted-foreground text-xs sm:text-sm lg:text-base">
                Orders that have at least one failed payment
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              ...
              <Button variant="outline" onClick={handleExportAll} className="w-full sm:w-auto h-10 sm:h-9">
          ...
          <Card className="overflow-hidden">
            <CardHeader className="px-3 sm:px-6">
              ...
              <CardDescription>Search and filter failed-payment orders</CardDescription>
            </CardHeader>
