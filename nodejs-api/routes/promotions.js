...
router.get('/', allowCustomerReadPromotions, [
  ...
  console.log('🔍 GET /promotions - User:', req.user?.role, 'CustomerId:', req.user?.customerId);
  ...
  if (req.user && req.user.role === 'CUSTOMER' && req.user.customerId) {
    console.log('✅ Applying customer filter for customerId:', req.user.customerId);
    where.AND.push({
      OR: [
    ...
    console.log('⚠️ No customer filter applied - User role:', req.user?.role, 'Has customerId:', !!req.user?.customerId);
  }
  ...
  console.log(`📊 Promotions returned: ${promotions.length} (Filtered from ${rawPromotions.length})`);
